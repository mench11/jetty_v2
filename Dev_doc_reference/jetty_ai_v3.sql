...
--
-- Host: localhost
-- Generation Time: Mar 10, 2025 at 01:46 PM
...
INSERT INTO `api_tokens` (`id`, `user_id`, `name`, `value`, `provider`, `created_at`, `last_used`, `usage_count`, `status`) VALUES
('token111-1111-1111-1111-111111111111', '33333333-3333-3333-3333-333333333333', 'OpenAI API Key', 'sk-dummy-openai-api-key-for-testing-purposes-only', 'openai', '2023-04-30 23:00:00', '2023-06-19 23:00:00', 42, 'active'),
('token222-2222-2222-2222-222222222222', '33333333-3333-3333-3333-333333333333', 'DeepSeek API Key', 'sk-dummy-deepseek-api-key-for-testing-purposes-only', 'deepseek', '2023-05-14 23:00:00', '2023-06-14 23:00:00', 27, 'active');
...
--
-- Stand-in structure for view `vw_api_tokens_with_users`
-- (See below for the actual view)
--
CREATE TABLE `vw_api_tokens_with_users` (
`id` char(36)
,`user_id` char(36)
...
,`name` varchar(255)
,`value` text
,`provider` enum('openai','deepseek','other')
...
--
-- Structure for view `vw_api_tokens_with_users`
--
DROP TABLE IF EXISTS `vw_api_tokens_with_users`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_api_tokens_with_users`  AS SELECT `t`.`id` AS `id`, `t`.`user_id` AS `user_id`, `u`.`name` AS `user_name`, `u`.`email` AS `user_email`, `t`.`name` AS `name`, `t`.`value` AS `value`, `t`.`provider` AS `provider`, `t`.`created_at` AS `created_at`, `t`.`last_used` AS `last_used`, `t`.`usage_count` AS `usage_count`, `t`.`status` AS `status` FROM (`api_tokens` `t` left join `users` `u` on(`t`.`user_id` = `u`.`id`)) ;
